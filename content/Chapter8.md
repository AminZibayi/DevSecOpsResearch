...
*   **Open Question:** Can we quantify the "Optimal Spec-to-Code Ratio"? Are there specific domains (e.g., cryptographic implementations, authentication middleware) where the ratio approaches 1:1, whereas boilerplate UI components can tolerate a 1:10 ratio? Furthermore, can we use AI to auto-generate the test suite *from* the spec to enforce the contract before code generation?
...
*   **Current State:** We use static SAST to evaluate the *output* of the AI, but we lack robust ways to evaluate the *reasoning* or the *agent*.
